...
async def autoDelete_settings(client, message):
    ...
    try:
            ...
            reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("Cʟᴏsᴇ ✖️", callback_data = "close")]])
            await message.reply(f"<b>! Eʀʀᴏʀ Oᴄᴄᴜʀᴇᴅ..\n<blockquote>Rᴇᴀsᴏɴ:</b> {e}</blockquote><b><i>Cᴏɴᴛᴀɴᴄᴛ ᴅᴇᴠᴇʟᴏᴘᴇʀ: @Heaven_Emperor</i></b>", reply_markup=reply_markup)
...
hidefsub_cmd = "hidefsub"

@Bot.on_message(filters.command(hidefsub_cmd) & filters.private & is_admin)
async def add_chat_cmd(client: Bot, message: Message):
    ...
    try:
        ...
        if matched_dict["delete"]:
            ...
            result = (
                ...
                f"<blockquote>{f'HIDE-FSUB-LINK-ID: <code>{recevied_id}</code>\n' if recevied_id else ''}"
