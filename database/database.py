...
class SidDataBase:
    ...
    def __init__(self, DB_URI, DB_NAME):
        ...
        self.__abstract_forcesub_data = self.database['abstract_force_sub']
    
    
    #ABSTRACT FORCE SUB MANAGEMENT
    async def abstract_force_sub(
        self, 
        chat_id: int|None = None, folder_link: str|None = None, 
        *,
        update: bool = False, delete: bool = False
    ) -> bool | tuple[int|None, str|None] | None:
        if update and delete:
            raise ValueError("update and delete both can't be selected at a time!")
        
        collection = self.__abstract_forcesub_data
        id_filter = {"_id": "ABS-FSUB"}

        if not (chat_id or folder_link):
            if delete:
                return (await collection.delete_one(id_filter)).deleted_count > 0

            result = await collection.find_one(id_filter)
            return (result.get("chat_id"), result.get("folder_link")) if result else None

        if update:
            data = {k: v for k, v in [("chat_id", chat_id), ("folder_link", folder_link)] if v}
